...
def calculate_score(list1):
        for item1 in list1:
            if item1 == 10 and item1 == 11 and len(list1) == 2:
                return 0
        if sum(list1) == 21 and 11 in list1 and len(list) == 2:
            list1.remove(11)
            list1.append(1)
        return sum(list1)
...
def play_game():
    ...
    while not is_game_over:
        ...
        print(f"    {user_cards},current_score = {user_sum}")
        print(f"    Computer card:{computer_cards[0]}")
        user_sum = calculate_score(user_cards)
        if user_sum == 0 or user_sum > 21 or computer_sum == 0:
            is_game_over = True
        else:  
            check = input("Do you want to draw another card:")
            if check == 'y':
                user_cards.append(deal_card())
            else:
                is_game_over = True
...
play_game()
should_continue = input("Do you want to play again, y for yes and n for no : ")
...
#If the computer has a blackjack (0), then the user loses. cpmputer wins 
...
#lets start making a blackjack project